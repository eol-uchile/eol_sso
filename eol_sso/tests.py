...
class GenerateUsernameTests(TestCase):
    """utils.generate_username with the structured-keys input build_user_data uses."""
...
class FindExistingUserTests(TestCase):
    ...
    def test_no_match_returns_none(self):
        self.assertIsNone(find_existing_user(["nobody@uchile.cl"]))

    def test_single_match_returned(self):
        u = User.objects.create_user("solo", "a@uchile.cl", "pass1234")
        self.assertEqual(find_existing_user(["a@uchile.cl"]), u)

    def test_multi_match_resolves_by_recent_last_login(self):
        ...
        result = find_existing_user(["a@uchile.cl", "b@uchile.cl"])
        self.assertEqual(result, recent)

    def test_never_logged_in_loses_tiebreak(self):
        User.objects.create_user("never", "a@uchile.cl", "pass1234")
    ...
    def test_excludes_already_linked_email(self):
        """
        An email already tied to an existing UchileOAuth2Backend social-auth
        identity is excluded from matching, so a different SSO identity can't
        get linked to an account someone else already claimed.
        """
        linked_user = User.objects.create_user("linked", "shared@uchile.cl", "pass1234")
        UserSocialAuth.objects.create(
            user=linked_user, provider=UchileOAuth2Backend.name, uid="111"
        )
        other_user = User.objects.create_user("other", "other@uchile.cl", "pass1234")

        result = find_existing_user(["shared@uchile.cl", "other@uchile.cl"])
...
class SyncProfileTests(TestCase):
    ...
    def test_reassigns_indiv_id_from_different_user(self):
        old_owner = User.objects.create_user("old", "old@uchile.cl", "pass1234")
        new_owner = User.objects.create_user("new", "new@uchile.cl", "pass1234")
        UserIndivId.objects.create(user=old_owner, indiv_id="999")

        link_indiv_id(new_owner, "999")

        self.assertFalse(UserIndivId.objects.filter(user=old_owner).exists())
        self.assertEqual(UserIndivId.objects.get(indiv_id="999").user, new_owner)
...
class ProvisionUserFromIndivIdTests(TestCase):
    ...
    def test_psa_linked_user_just_syncs(self, mock_api):
        ...
        user, _ = provision_user_from_indiv_id(INDIV_ID, existing_user=linked)

        self.assertEqual(user.pk, linked.pk)
        self.assertEqual(User.objects.count(), before)
        self.assertTrue(UserIndivId.objects.filter(user=linked).exists())

    def test_returning_name_synced(self, mock_api):
        """
        On a returning-user call: email/username are read from the existing
        account and left untouched, even if persona_data now reports a
        different email but first/last/fullname are always recomputed
        fresh from persona_data, since name corrections should propagate.
        """
        existing = User.objects.create_user(
            "returning_user", "original@uchile.cl", "pass1234",
            first_name="Old", last_name="Name",
        )
        mock_api.return_value = {
            **FAKE_PERSONA,
            "email_principal": "different@uchile.cl",
            "nombres": "Updated",
            "paterno": "NewLast",
            "materno": "NewSecond",
        }

        user, user_data = provision_user_from_indiv_id(INDIV_ID, existing_user=existing)

        self.assertEqual(user.pk, existing.pk)
        self.assertEqual(user_data["email"], "original@uchile.cl")
        self.assertEqual(user_data["username"], "returning_user")
        self.assertEqual(user_data["first_name"], "Updated")
        self.assertEqual(user_data["last_name"], "NewLast NewSecond")
        self.assertEqual(user_data["fullname"], "Updated NewLast NewSecond")
